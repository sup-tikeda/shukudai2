...
同じ `DataTable` で列分けしています**。当初は件数が少ないため `Row`（1行にまとめる表示）の
ままにしていましたが、一覧画面と体裁が揃っていないと分かりづらいため統一しました。

- 顧客詳細の「保有車両」「案件」「見積・請求」
- 車両詳細の「案件」
- 案件詳細の「見積・請求」
- 見積・請求詳細の「明細項目」（項目名／数量／単価／税率／小計）
- 設定＞社員マスタ（社員コード／氏名／権限／役職・資格／入社日／状態／連絡先）

案件の残り日数表示（`RemainingDaysLabel`）は、案件一覧・案件詳細・車両詳細（この車両の
案件一覧）の3箇所で使うため共通部品にしています。
...
- **本文の横幅には上限を付けていません**（`max-w-none`。一覧・詳細・設定・ダッシュボードの
  全画面共通）。サイドバー導入後、`max-w-5xl`→`max-w-6xl`と上限を上げても
  広い画面では余白ばかり目立ったため、上限そのものを外しました
  （[layout.tsx](src/components/ui/layout.tsx)）。
  ただし**顧客・車両・案件の「基本情報」（`DetailList`）だけは例外**です。
  ラベルと値の短い組を並べるだけなので、列数を増やして幅いっぱいに広げると
  かえって間延びして見えました。表（`DataTable`）を使う一覧部分は引き続き
  幅いっぱいまで広げます。
- **`DetailList`（基本情報）も見た目を`DataTable`に揃え、ラベル｜値の表**として
  描画します。以前はラベルを上・値を下に積む2列グリッドでしたが、一覧の表と体裁が
  揃っていないと分かりづらいため統一しました。
  - **短い項目は無理に1行1項目にせず、横に並べて「通常の項目全体が2段に収まる」
    ように組みます**（ラベル｜値｜ラベル｜値…）。1行に並べる組数は項目数に応じて
    自動で決まり、項目数が多い画面（車両情報の8項目など）ほど1行に並べる数を
    増やします（上限4組）。項目数が少ない画面（顧客の基本情報など）は
    これまでどおり2組のままです。
  - **`wide`を付けた項目（住所・備考など長文になりうるもの）だけ単独で1行**にし、
    値の列を残り全部まで広げます。1行に満たなかった最後の組も同様に広げます。
  - この組み方は`DetailList`が`DetailItem`の子要素を読み取って自動で決めます。
    `DetailItem`自体はDOMに何も描画しません（`DetailList`側で表の行を組み立てるため）。
    呼び出し側（顧客・車両・案件の各詳細画面）は`<DetailItem label="...">`を並べるだけで、
    何組ずつ並べるかを自分で決める必要はありません。
  - **表に`w-full`（や`min-w`）は付けていません**。付けると値の列に余った幅が
    押し付けられ、短い値の右側にだけ大きな空白ができてしまいます。
    幅を指定しない表は中身の長さに合わせて縮む（HTMLの既定の挙動）ため、値が短ければ
    表そのものも小さくなります。項目を横に並べるほど自然と幅は広がるので、
    「横に広げる」意図とも両立できます。長い住所などが来た時のために`max-w-6xl`だけ
    上限として残しています。
  - **余白・文字サイズも`DataTable`の行（`px-3 py-2`・13px）と揃え**、行の高さの密度まで
    一覧の表と同じにしています。
