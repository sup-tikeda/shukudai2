...
- **一覧画面（`fill`）は本文の横幅の上限を外しています**（`max-w-none`）。列数の多い表を
  広い画面いっぱいまで使えるようにするためです。詳細・設定・ダッシュボードは読みやすさを
  優先し、`max-w-6xl` の上限を残しています（[layout.tsx](src/components/ui/layout.tsx)）。
