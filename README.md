...
- **`DetailList`（基本情報）も見た目を`DataTable`に揃え、ラベル｜値の表**として
  ...
  - **短い項目は無理に1行1項目にせず、2つずつ横に並べて1行にします**
  （ラベル｜値｜ラベル｜値）。車両情報のように項目数が多い画面で、
  縦にひたすら間延びするのを防ぐためです。
  - **`wide`を付けた項目（住所・備考など長文になりうるもの）だけ単独で1行**にし、
    値の列を残り全部まで広げます。項目数が奇数で相方がいない項目も同様に広げます。
